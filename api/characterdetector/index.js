...
async function retrievePictures(version) {
    ...
    const dirCont = await getFiles(`../images/thumbnail/${version}`)
    return dirCont
...
async function computeMatrice(files, size, version) {
    ...
    await sharp({
        ...
        .composite(pictures)
        .png()
        .toFile(`../images/thumbnail/cropped/${filename}.png`)

    console.timeEnd(`Computation of ${filename} pictures`)

    await writeFile(`../images/thumbnail/cropped/${filename}.json`, JSON.stringify(files.map(f => f.id)))
}
